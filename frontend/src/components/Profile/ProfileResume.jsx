        ...
        <section className='bg-black w-full h-auto min-h-profile px-4 py-10 flex flex-col gap-12 lg:h-profile lg:w-9/12 lg:max-h-profile lg:absolute lg:right-0 lg:top-32 lg:overflow-y-auto 2xl:w-10/12'>
            <section className='flex flex-col items-center gap-8 lg:flex-row lg:items-start'>
                <section className='w-full max-w-96 flex justify-center lg:justify-start lg:w-1/3 lg:max-w-60'>
                    <img src={`https://github.com/davibcavalcante.png`} alt={`Foto de perfil de ${user.username}`} className='w-1/2 rounded-full border-2 border-white animate-rotate-y animate-ease-in-out lg:w-full' />
                </section>
                <section className='flex flex-col gap-8'>
                    <section className='flex flex-col gap-2'>
                        <h1 className='text-white text-center text-3xl font-poppins font-semibold lg:text-start'>{user.username}</h1>
                        <h2 className='text-white text-center text-xl font-poppins font-light lg:text-start'>{capitalizeText(user.office)}</h2>
                    </section>
                    <section>
